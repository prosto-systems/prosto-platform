...
- In current repository state, lint and architecture-policy commands plus `test:contracts` are available in root `package.json`; `validate:runtime-policy` and `test:lifecycle-determinism` remain placeholders by phase design.
