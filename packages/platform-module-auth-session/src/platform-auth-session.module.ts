...
} from '@prosto/platform-adapter-auth-session';
import {
...
import type {
  ...
  IPlatformAuthSessionModuleFacade,
...
import { auth_session_create_tables1710000000000 } from '@/migrations/index.js';
import {
  DeferredResolver,
 ...
 */
export class PlatformAuthSessionModule implements IPlatformModule {
  readonly facade: IPlatformAuthSessionModuleFacade;
  ...
  constructor(private readonly _config: IPlatformAuthSessionModuleConfig) {
  ...
  init(context: IPlatformModuleContext): void {
    context.persistence?.descriptors?.register(context.moduleId, {
      ...
      payload: createTypeOrmPersistenceDescriptor({
        ...
        migrations: [auth_session_create_tables1710000000000],
      }),
    });
  }
