...
describe('TypeOrmOidcSessionStore', (): void => {
  it('creates portable session tables through the migration', async (): Promise<void> => {
    ...
    await expect(
      dataSource.query(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='auth_session_sessions'",
      ),
    ).resolves.toHaveLength(1);
    await expect(
      dataSource.query(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='auth_session_transactions'",
      ),
    ).resolves.toHaveLength(1);
  });
