...
describe('TopologicalSorter', () => {
  ...
  it('orders modules by dependency', () => {
    ...
    expect(result.orderedModules.map((moduleItem) => moduleItem.manifest.id)).toEqual([
      'a',
      'b',
      'c',
    ]);
  ...
  it('tracks missing required dependencies', () => {
    ...
    expect(result.orderedModules.map((moduleItem) => moduleItem.manifest.id)).toEqual(['a']);
  });
