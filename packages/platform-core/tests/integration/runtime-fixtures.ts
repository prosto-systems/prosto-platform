import type { IModuleContext, IPlatformModule, IPlatformModuleManifest } from '@prosto/platform-sdk';

export function createManifest(input: Partial<IPlatformModuleManifest> & Pick<IPlatformModuleManifest, 'id'>): IPlatformModuleManifest {
  return {
    id: input.id,
...
export class TestModule implements IPlatformModule {
  ...
  async stop(_ctx: IModuleContext): Promise<void> {
    ...
    if (this._behavior.stopDelayMs > 0) {
